...
void DescrVBOAtribs::crearVBO() 
{
   ...
   // Práctica 1: crear el VBO de atributos en la GPU
   // Diapositiva 119 del tema 1 de Teoría
...
void DescrVBOInds::crearVBO( )
{
   ...
   // Práctica 1: crear el VBO de índices en la GPU
   // Diapositiva 120 del Tema 1
   // Añadir el código para crear el VBO en la GPU y transferir los datos
...
void DescrVAO::crearVAO()
{
   ...
   // Práctica 1: crear el VAO y enviar datos de VBOs
   // Escribir el código que crea el VAO en la GPU y registra (asocia) 
   // los VBOs en este VAO
   
   // 1. Crear el nombre de VAO y activarlo como VAO actual (hacer 'bind')
   glGenVertexArrays(1, &array); // produce array > 0
   assert(array>0);
   glBindVertexArray(array);     // activa el VAO
