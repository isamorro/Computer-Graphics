...
NodoDiscoP4::NodoDiscoP4()
{
   ...
   // Si genero la textura con XY me la genera con cuadros muy pequeños
   // Es claro que si la genero con XZ, no hace nada
...
) : Textura ("text-madera.jpg")
{
   if (orientacion) {
      ...
      coefs_s[1] = 1.0; coefs_s[0] = 0.0;
      coefs_t[1] = 0.0; coefs_t[0] = 1.0;
   }
}
...
// *********************************************************************
// EJERCICIOS OPCIONALES PRÁCTICA 5
...
MiEsferaE1::MiEsferaE1(unsigned i, unsigned j){
   fila = i;
   columna = j;
   agregar(new Esfera(20,20)); // esfera en Malla-Revol
}

bool MiEsferaE1::cuandoClick(const glm::vec3 & centro_wc) {
   std::cout << "Se ha seleccionado la esfera número " << columna + 1 << " de la fila número " << fila + 1 << std::endl;
}
...
MiEsferaE2::MiEsferaE2(){
   ...
   ponerColor(glm::vec3(1.0,1.0,1.0)); // blaco
   agregar(new Esfera(20,20)); // De malla-revol
